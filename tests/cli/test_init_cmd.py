...
@patch("dem.core.commands.init_cmd.DevEnv.from_descriptor_path")
def test_execute(mock_from_descriptor_path, mock_confirm, mock_stdout_print, mock_isdir) -> None:
    # Test setup
    ...
    mock_from_descriptor_path.return_value = mock_dev_env
    ...
    mock_from_descriptor_path.assert_called_once_with(
        f"{mock_project_path}/.axem/dev_env_descriptor.json", mock_platform.hosts
    )
...
@patch("dem.core.commands.init_cmd.stderr.print")
@patch("dem.core.commands.init_cmd.DevEnv.from_descriptor_path")
def test_execute_missing_descriptor(mock_from_descriptor_path, mock_stderr_print, mock_isdir) -> None:
    # Test setup
    mock_platform = MagicMock()
    mock_project_path = "/path/to/project"
    mock_from_descriptor_path.side_effect = FileNotFoundError
    mock_isdir.return_value = True
    mock_platform.hosts = MagicMock()
...
@patch("dem.core.commands.init_cmd.typer.confirm")
@patch("dem.core.commands.init_cmd.DevEnv.from_descriptor_path")
def test_execute_reinit_installed(mock_from_descriptor_path, mock_confirm, mock_stdout_print, mock_isdir) -> None:
    # Test setup
    mock_platform = MagicMock()
    mock_project_path = "/path/to/project"
    mock_dev_env_name = "test_dev_env"
...
def test_execute_reinit_installed_uninstall_fails(mock_DevEnv, mock_confirm, mock_stderr_print, 
    ...
    # Check expectations
    mock_isdir.assert_called_once_with(mock_project_path)
    mock_DevEnv.assert_called_once_with(
        f"{mock_project_path}/.axem/dev_env_descriptor.json", mock_platform.hosts
    )
    mock_confirm.assert_has_calls([call("Would you like to re-init the Dev Env? All local changes will be lost!", abort=True),
                                   call("The Development Environment is installed, so it can't be deleted. Do you want to uninstall it first?", abort=True)])
